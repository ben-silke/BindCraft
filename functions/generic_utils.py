...
def check_n_trajectories(design_paths, advanced_settings):
    n_trajectories = [
        f for f in os.listdir(design_paths["Trajectory/Relaxed"]) if f.endswith(".pdb")
    ]
...
def perform_advanced_settings_check(advanced_settings, bindcraft_folder):
    ...
    if bindcraft_folder == "colab":
        ...
        advanced_settings["dalphaball_path"] = (
            "/content/bindcraft/functions/DAlphaBall.gcc"
        )
    else:
...
def calculate_averages(statistics, handle_aa=False):
    ...
    if handle_aa:
        aa_averages = {
            aa: round(total / len(statistics), 2) for aa, total in aa_sums.items()
        }
...
def check_filters(mpnn_data, design_labels, filters):
    ...
    for label, conditions in filters.items():
        ...
        if (
            label == "Average_InterfaceAAs"
            or label == "1_InterfaceAAs"
            or label == "2_InterfaceAAs"
            or label == "3_InterfaceAAs"
            or label == "4_InterfaceAAs"
            or label == "5_InterfaceAAs"
        ):
